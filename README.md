...
The user has the option to enter an API Key for their AI model of choice and generate deeper insights about their spending patterns, balances, and transfer/savings recommendations. Click **✦ AI Insights** in the header to slide open the optional panel and see the AI's "advice," which is often accurate but sometimes makes erroneously assumptions. In that case, the Corrections panel at the bottom lets you feed the AI specific facts about your finances to fix its accuracy or provide additional context that help it generate better recommendations. If no API keys are configured this panel remains hidden out of view. 
