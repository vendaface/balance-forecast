...
# Usage: double-click run.cmd, or: powershell -ExecutionPolicy Bypass -File run.ps1
#
...
function Open-Startup {
    ...
    if ($Params) {
        ...
        (Get-Content $tmp -Raw) -replace '</head>', $inject | Set-Content $tmp -Encoding UTF8
    }
...
# Allow forcing the no-python error for testing: $env:SIMULATE_NO_PYTHON=1
if ($env:SIMULATE_NO_PYTHON -eq '1') { $script:PYTHON = $null }
...
if (-not $script:PYTHON) {
    ...
    Write-Host ''
    Write-Host '  Option 1 — winget (Windows 11 / updated Windows 10):'
    ...
    Write-Host ''
    Write-Host '  After installing, run run.cmd again.'
    Write-Host ('━' * 52)
    Write-Host ''
    exit 1
}
...
Write-Status 'starting' 2 'Starting Butterfly Effect…' 0 $TotalSteps
Start-StatusServer
...
if (-not $VenvOk) {
    $Step++
    Write-Status 'venv' 5 'Creating Python environment…' $Step $TotalSteps
    ...
    & $script:PYTHON -m pip install --upgrade pip `
        --target (Join-Path $Venv 'Lib\site-packages') --quiet 2>$null
...
# ── Dependencies ──────────────────────────────────────────────────────────────
$Step++
Write-Status 'pip' 18 'Installing Python packages…' $Step $TotalSteps
...
$pipJob = Start-Job -ScriptBlock {
    ...
    & $pip install -q -r $req
    exit $LASTEXITCODE
} -ArgumentList $pip, (Join-Path $ScriptDir 'requirements.txt')

while ($pipJob.State -eq 'Running') {
    Write-Host -NoNewline '.'
    Start-Sleep -Seconds 1
}
$pipResult = Receive-Job $pipJob -Wait
Remove-Job $pipJob

if ($LASTEXITCODE -ne 0 -or $pipJob.State -eq 'Failed') {
    Write-Host ' failed.'
    Open-Startup 'e=pipfail&os=windows'
    Write-Host 'Error: dependency install failed. See the browser window for details.'
    exit 1
}
Write-Host ' done.'
...
if ($NeedPlaywright) {
    $Step++
    Write-Status 'playwright' 47 'Downloading Chromium browser (~250 MB)…' $Step $TotalSteps
    Write-Host ''
    ...
    $pwReceive = Receive-Job $pwJob -Wait
    $pwExit    = if ($pwJob.State -eq 'Failed') { 1 } else { 0 }
    Remove-Job $pwJob

    Write-Host "  [debug] playwright log:"
    Get-Content $pwLog -ErrorAction SilentlyContinue | ForEach-Object { Write-Host "    $_" }
...
# ── Launch ────────────────────────────────────────────────────────────────────
$Step++
Write-Status 'starting' 96 'Starting server…' $Step $TotalSteps
